...
public class GiftMenu extends Menu {
	...
	public void OnPageOpened(int page) {
		...
		Inventory gui = createInventory(getSize(), me.davidml16.baul.utils.Colorize.format(translateTitleVariables(guiLayout.getRawMessage("Title"), cubeletTypes.size())));
		...
		if (cubeletTypes.size() > 0) {
			...
			for (CubeletType cubeletType : cubeletTypes) {
				...
				for (String line : guiLayout.getMessageList("Items.Cubelet.Lore")) {
					lore.add(Utils.translate(line.replaceAll("%baul_available%", String.valueOf(amount))));
				}

				ItemStack item = new ItemBuilder(cubeletType.getIcon()
						.clone()).setName(Utils.translate(guiLayout.getMessage("Items.Cubelet.Name")
						.replace("%baul_name%", cubeletType.getName()))).setLore(lore).toItemStack();
