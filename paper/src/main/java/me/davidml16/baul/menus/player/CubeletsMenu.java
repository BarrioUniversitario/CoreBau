...
import me.davidml16.baul.interfaces.CubeletDateComparator;
import me.davidml16.baul.interfaces.CubeletTypeComparator;
import me.davidml16.baul.menus.player.crafting.CraftingMenu;
import me.davidml16.baul.menus.player.gifts.GiftPlayerMenu;
import me.davidml16.baul.menus.player.rewards.RewardsPreviewMenu;
import me.davidml16.baul.objects.*;
...
public class CubeletsMenu extends Menu {
	...
	public void OnPageOpened(int page) {
		...
		Inventory inventory = createInventory(getSize(), Colorize.format(translateTitleVariables(guiLayout.getRawMessage("Title"), cubelets.size())));
		...
		if (cubelets.size() > 0) {
			...
			for (Cubelet cubelet : cubelets) {
				...
				// Skip if type is null (shouldn't happen but prevents NPE)
				if (type == null) {
					continue;
				}
				...
				if (cubelet.getExpire() < 0 || cubelet.getExpire() > System.currentTimeMillis()) {
	                       ...
	                       for (String line : type.getLoreAvailable()) {
	                           String formattedLine = line.replaceAll("%received%", TimeUtils.millisToLongDHMS(System.currentTimeMillis() - cubelet.getReceived()));
	                           formattedLine = formattedLine.replaceAll("%expires%", (cubelet.getExpire() >= 0 ? TimeUtils.millisToLongDHMS(cubelet.getExpire() - System.currentTimeMillis()) : guiLayout.getMessage("NoneExpiration")));
	                           lore.add(MiniMessageUtils.format(formattedLine));
	                       }
		...
		if (slot >= (size - 9) && slot <= size) {
			...
			if (getMain().getPlayerDataHandler().getData(player.getUniqueId()).getCubelets().size() > 0) {
				...
				if (slot == getCenterSlot()) {
					...
					player.closeInventory();
					Colorize.sendShopMessage(player);
