...
public class Reward {
    ...
    private List<CosmeticUnlockObject> cosmetics;
    ...
    public List<CosmeticUnlockObject> getCosmetics() {
        if (cosmetics == null) cosmetics = new ArrayList<>();
        return cosmetics;
    }

    public void setCosmetics(List<CosmeticUnlockObject> cosmetics) { this.cosmetics = cosmetics; }

    public CosmeticUnlockObject getCosmetic(String id) {
        return cosmetics.stream().filter(cu -> cu.getId().equalsIgnoreCase(id)).findFirst().orElse(null);
    }

    public void recreateCosmetics() {
        int i = 0;
        for (CosmeticUnlockObject cu : getCosmetics()) {
            cu.setId("cosmetic-" + i);
            i++;
        }
    }
