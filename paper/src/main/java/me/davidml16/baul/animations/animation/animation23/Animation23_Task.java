...
public class Animation23_Task extends Animation {
	...
	public void onStart() {
		...
		spawnItemsRunnable = Bukkit.getScheduler().runTaskTimer(getMain(), () -> {
			if(items.size() < 12) {
				Animation23_Item item = new Animation23_Item(getMain(), getMain().getCubeletRewardHandler().processReward(getCubeletType(),
						getMain().getPlayerDataHandler().getData(getCubeletBox().getPlayerOpening().getUuid())), getCubeletBox().getOriginalLocation().clone().add(0.5, 2, 0.5), circleLocations, circleInitialStep, headChecker);
				item.Tick();
