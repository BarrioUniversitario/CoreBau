...
public class ExecuteGiveKey {
    ...
    private void giveKey(CommandSender sender, Player target, CubeletType cubeletType, int amount) {
        ...
        String msg = main.getLanguageHandler().getRawMessage("Commands.Cubelets.GiveKey.Give");
        msg = msg.replaceAll("%amount%", Integer.toString(amount));
        msg = msg.replaceAll("%baul%",  cubeletType.getName());
        msg = msg.replaceAll("%player%", target.getName());
        sender.sendMessage(me.davidml16.baul.utils.Colorize.format(msg));

        String msgReceived = main.getLanguageHandler().getRawMessage("Commands.Cubelets.GiveKey.Received");
        msgReceived = msgReceived.replaceAll("%amount%", Integer.toString(amount));
        msgReceived = msgReceived.replaceAll("%baul%",  cubeletType.getName());
        target.sendMessage(me.davidml16.baul.utils.Colorize.format(msgReceived));
