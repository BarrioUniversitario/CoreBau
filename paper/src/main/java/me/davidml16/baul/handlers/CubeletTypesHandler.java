...
import me.davidml16.baul.utils.TimeAPI.TimeAPI;
...
public class CubeletTypesHandler {
				...
				if (validTypeData(config)) {
					...
					if (!config.contains("type.key")) {
						...
						ItemStack key = new ItemBuilder(XMaterial.TRIPWIRE_HOOK.parseMaterial(), 1).addGlow()
								.setName(me.davidml16.baul.utils.Colorize.format(Colorize.escapeTags(config.getString("type.name")) + "'s key"))
								.setLore(Arrays.asList(me.davidml16.baul.utils.Colorize.format("&8&oGo to a Cubelets Machine to"), me.davidml16.baul.utils.Colorize.format("&8&oopen this key."), "", me.davidml16.baul.utils.Colorize.format("&a&lLeft-Click: &6Preview rewards"), me.davidml16.baul.utils.Colorize.format("&a&lRight-Click: &6Open cubelet key")))
								.toItemStack();
	...
	public void printLog() {
		...
		Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("  &eLoading cubelets:"));
		...
		for (CubeletType cubeletType : types.values()) {
			...
			Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format(log));

		}

		if (types.size() == 0) Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("    &cNo cubelets has been loaded!"));
		Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format(""));
