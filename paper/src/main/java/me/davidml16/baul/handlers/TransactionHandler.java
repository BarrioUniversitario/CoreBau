...
import me.davidml16.baul.api.CubeletReceivedEvent;
...
public class TransactionHandler {
    ...
    public void removePoints(UUID uuid, int amount) throws SQLException {
        ...
        main.getDatabaseHandler().getPlayerLootPoints(uuid, done -> {
            ...
            if((done - amount) < 0) {
                main.getDatabaseHandler().setPlayerLootPoints(uuid, 0, syncTask);
            }
            else {
                main.getDatabaseHandler().setPlayerLootPoints(uuid, done - amount, syncTask);
            }
        });
