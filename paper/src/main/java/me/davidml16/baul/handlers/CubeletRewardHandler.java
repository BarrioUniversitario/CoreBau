...
public class CubeletRewardHandler {
                ...
                for (String rewardid : config.getConfigurationSection("type.rewards")
                    ...
                    if (validRewardData(config, rewardid)) {
                        ...
                        if (cubeletType.getRarities()
                            ...
                            List<CosmeticUnlockObject> cosmetics = new ArrayList<>();
                            ...
                            if (config.contains("type.rewards." + rewardid + ".cosmetic"))
                                getRewardCosmetics(config, cosmetics, rewardid);
    ...
    private void getRewardCosmetics(FileConfiguration config, List<CosmeticUnlockObject> list, String rewardid) {
        int i = 0;
        if (config.get("type.rewards." + rewardid + ".cosmetic") instanceof ArrayList) {
            for (String cosmeticId : config.getStringList("type.rewards." + rewardid + ".cosmetic")) {
                list.add(new CosmeticUnlockObject("cosmetic-" + i, cosmeticId.toLowerCase()));
                i++;
            }
        } else {
            String cosmeticId = config.getString("type.rewards." + rewardid + ".cosmetic");
    ...
    public Reward processReward(CubeletType cubeletType) {
        return processReward(cubeletType, null);
    }

    /**
     * Adaptive-luck-aware variant. When {@code profile} is non-null, applies a
     * hidden weight bonus to rarities whose base chance is &lt; 10% based on the
     * player's {@code unluckyOpens} counter. The counter resets to 0 when a
     * &lt;10% rarity is finally picked, and increments otherwise.
     */
    public Reward processReward(CubeletType cubeletType, Profile profile) {
        double luckBonus = profile == null ? 0.0 : Math.min(profile.getUnluckyOpens() * 0.01, 5.0);

        List<Rarity> rarities = getAvailableRarities(cubeletType);
        Rarity randomRarity = chooseOnWeight(rarities, luckBonus);

        if (profile != null) {
            if (randomRarity.getChance() < 10.0) profile.setUnluckyOpens(0);
            else profile.setUnluckyOpens(profile.getUnluckyOpens() + 1);
        }
        ...
        if (cubeletType.getRewards().containsKey(randomRarity.getId())) {
            ...
            if (rewards.size() <= 0) return processReward(cubeletType, profile);
    ...
    public void giveReward(CubeletMachine cubeletMachine, CubeletType cubeletType, Reward reward) {
        ...
        // Loot history shows the COSMETIC itself when the reward grants one — its
        // display name and icon material — instead of whatever placeholder item the
        // admin configured on the reward. Falls back to the reward's name/icon when
        // the reward has no cosmetic (items / commands / permissions rewards).
        String historyName = reward.getName();
        org.bukkit.inventory.ItemStack historyIcon = reward.getIcon();
        if (!reward.getCosmetics().isEmpty() && main.getCosmeticRegistry() != null) {
            me.davidml16.baul.cosmetics.Cosmetic first = main.getCosmeticRegistry()
                    .getById(reward.getCosmetics().get(0).getCosmeticId());
            if (first != null) {
                historyName = first.getDisplayName();
                com.cryptomorin.xseries.XMaterial xm = com.cryptomorin.xseries.XMaterial
                        .matchXMaterial(first.getIconMaterial())
                        .orElse(com.cryptomorin.xseries.XMaterial.PAPER);
                org.bukkit.inventory.ItemStack ic = xm.parseItem();
                if (ic != null) historyIcon = ic;
            }
        }

        RewardHistory rewardHistory = new RewardHistory(reward.getRewardUUID(), historyName, historyIcon);
        ...
        if (!reward.getCosmetics().isEmpty()
                && main.getCosmeticRegistry() != null
                && main.getCosmeticRegistry().isEnabled()) {
            Profile cosmeticProfile = target != null ? main.getPlayerDataHandler().getData(target) : null;
            for (CosmeticUnlockObject cu : reward.getCosmetics()) {
                String cosmeticId = cu.getCosmeticId();
                if (main.getCosmeticRegistry().getById(cosmeticId) == null) {
                    main.getLogger().warning("Reward " + reward.getId() + " grants unknown cosmetic id: " + cosmeticId);
                    continue;
                }
                if (cosmeticProfile != null) cosmeticProfile.getOwnedCosmetics().add(cosmeticId);
                main.getDatabaseHandler().addOwnedCosmetic(playerUUID, cosmeticId, null);
            }
            SyncManager cosmeticSync = main.getSyncManager();
            if (cosmeticSync != null && cosmeticSync.isEnabled()) cosmeticSync.syncCosmeticUnlock(playerUUID);
        }
    ...
    public boolean isDuplicated(CubeletMachine cubeletMachine, Reward reward) {
        ...
        if (lootHistory != null) return true;

        for (CosmeticUnlockObject cu : reward.getCosmetics()) {
            if (profile.ownsCosmetic(cu.getCosmeticId())) return true;
        }
    ...
    public Rarity chooseOnWeight(List<Rarity> items) {
        return chooseOnWeight(items, 0.0);
    }

    /**
     * Weighted random choice with optional adaptive-luck bonus. Rarities below
     * 10% base chance get their weight multiplied by {@code (1 + luckBonus)}.
     */
    public Rarity chooseOnWeight(List<Rarity> items, double luckBonus) {
        double completeWeight = 0.0;
        for (Rarity rarity : items) completeWeight += effectiveWeight(rarity, luckBonus);
        double r = Math.random() * completeWeight;
        double countWeight = 0.0;
        for (Rarity item : items) {
            countWeight += effectiveWeight(item, luckBonus);
            if (countWeight >= r) return item;
        }
        throw new RuntimeException("Should never be shown.");
    }

    private double effectiveWeight(Rarity r, double luckBonus) {
        return r.getChance() < 10.0 ? r.getChance() * (1.0 + luckBonus) : r.getChance();
    }
