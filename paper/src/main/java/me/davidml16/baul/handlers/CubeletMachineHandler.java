...
import me.davidml16.baul.effects.MachineEffectModel;
...
public class CubeletMachineHandler {
    ...
    public void loadMachines() {
        ...
        Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("  &eLoading machines:"));
        ...
        if (config.contains("machines")) {
            if (config.getConfigurationSection("machines") != null) {
                for (int i = 1; i <= config.getConfigurationSection("machines").getKeys(false).size(); i++) {
                    ...
                    if (worldObj == null) {
                        Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("    &cWorld '" + world + "' not loaded! Attempting to load it."));
                        WorldCreator worldCreator = new WorldCreator(world);
                        worldObj = Bukkit.createWorld(worldCreator);

                        if (worldObj == null) {
                            Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("    &cWorld '" + world + "' does not exist! Skipping machine loading."));
                            continue;
                        }
                    }
        ...
        if (machines.size() == 0) Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("    &cNo Cubelet Machines has been loaded!"));
        else Main.log.sendMessage(me.davidml16.baul.utils.Colorize.format("    &b" + machines.size() + " &aCubelet Machines loaded!"));
